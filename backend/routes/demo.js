...
router.get("/question", userMiddleWare, async (req, res) => {
  try {
    ...
    if (!course || !course.questions.length) {
      return res.status(404).json({
        error: "No questions available for the course linked to the demo test.",
      });
    }
...
router.post("/submit", userMiddleWare, async (req, res) => {
  try {
    const { questionId, language, version, code, codeLength, keyStrokeCount } =
...
router.get("/viva-questions", userMiddleWare, async (req, res) => {
  try {
    const demoTest = await Test.findOne({ course: "demo" }).populate(
      "courseId"
    );
    ...
    if (!course || !course.vivaQuestions || course.vivaQuestions.length === 0) {
      return res
        .status(404)
...
router.post("/viva-submit", userMiddleWare, async (req, res) => {
  try {
